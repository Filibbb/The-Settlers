...
public class Settlement extends AbstractInfrastructure {
    ...
    private static boolean canBuild(Player owner, Point coordinates, SettlersBoard board) {
        return (board.hasCorner(coordinates) && board.getCorner(coordinates) == null && board.getNeighboursOfCorner(coordinates).isEmpty() && hasRoad(owner, coordinates, board) && owner.checkLiquidity(Structure.SETTLEMENT) && (owner.hasEnoughInStructureStock(Structure.SETTLEMENT)));
    }
